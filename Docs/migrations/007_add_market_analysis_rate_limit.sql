-- =============================================================================
-- Migración 007 — Límite de peticiones del análisis de mercado (Fase 8)
-- Motor: PostgreSQL (Supabase)
--
-- Cómo aplicarla:
--   Supabase Dashboard -> SQL Editor -> pega este archivo -> "Run".
--   (Es idempotente: CREATE OR REPLACE de la función de la migración 004.)
--
-- Qué hace:
--   Añade `/api/instruments/analyze` a los topes por ruta de check_rate_limit()
--   con un límite de 10 peticiones por minuto, el mismo que la retroalimentación
--   de quizzes: es otra llamada al modelo de IA y sin tope propio caería en el
--   valor por defecto de 30/min, tres veces más caro.
--
--   Aplicarla es OPCIONAL para que la funcionalidad arranque (sin ella rige el
--   tope por defecto), pero recomendable antes de abrirla a usuarios reales.
--
--   El resto de la función es idéntico a la migración 004: solo cambia el CASE
